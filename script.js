...
playButton.addEventListener("click", () => {
    if (isPlaying) return; // Предотвращаем повторное нажатие

    isPlaying = true;
    playButton.disabled = true; // Блокируем кнопку "Играть"
    ...
    const playMelody = () => {
        ...
        // Если включен цикл, запускаем мелодию заново
        if (isLooping) {
            setTimeout(playMelody, playbackQueue.length * playbackSpeed);
        } else {
            // Если цикл выключен, разблокируем кнопку после завершения
            setTimeout(() => {
                isPlaying = false;
...
function stopPlayback() {
    ...
    playButton.disabled = false; // Разблокируем кнопку "Играть"
    clearTimeouts(); // Очищаем все запланированные действия
}

// Функция для очистки запланированных воспроизведений
function clearTimeouts() {
    let id = window.setTimeout(() => {}, 0);
    while (id--) {
        window.clearTimeout(id); // Очищаем все таймеры
    }
}

// Обновляем функцию воспроизведения для учета кнопки "Стоп"
playButton.addEventListener("click", () => {
    if (isPlaying) return;
    ...
    const playMelody = () => {
        playbackQueue.forEach((item, index) => {
            ...
            setTimeout(() => {
                if (!isPlaying) return; // Прекращаем, если воспроизведение остановлено
                playNote(note, duration);
            }, index * playbackSpeed);
        });
...
speedControl.addEventListener("input", (e) => {
    const maxSpeed = parseInt(speedControl.max, 10); // Максимальное значение ползунка
    const minSpeed = parseInt(speedControl.min, 10); // Минимальное значение ползунка
    const value = parseInt(e.target.value, 10); // Текущее значение ползунка

    // Инвертируем значение: чем больше скорость, тем меньше задержка
    playbackSpeed = maxSpeed - value + minSpeed;

    console.log(`Скорость воспроизведения: ${playbackSpeed} мс`);
});
